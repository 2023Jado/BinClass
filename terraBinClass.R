...
sample_points <- function(ndvi_composite, num_samples, unchanged_pixels) {
  ...
  sampled_points <- terra::xyFromCell(ndvi_composite, sample_indices)
...
classify_ndvi_image <- function(ndvi_image, rf_model) {
  ...
  # Create a SpatRaster with the predictions
  classified_raster <- ndvi_image
  values(classified_raster) <- as.numeric(prediction)
